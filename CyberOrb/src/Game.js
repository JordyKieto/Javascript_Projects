...
Ball.Game.prototype = {
    create: function() {
        ...
        this.initLevels();
        this.showLevel(1);
        this.keys = this.game.input.keyboard.createCursorKeys();  // will give an object event handlers
    ...
    for(var i=0; i<this.maxLevels; i++) {
        ...
        for(var e=0; e<this.levelData[i].length; e++) {
            ...
            newLevel.create(item.x, item.y, 'element-'+item.t); // creates new item in groups 3rd param is IMG
        }
    ...
    showLevel: function(level) {
        ...
        if(this.levels[lvl-2]) {
            this.levels[lvl-2].visible = false;     // objects in lvl array default invisible
        }
    ...
    update: function() {
        if(this.keys.left.isDown) {
            this.ball.body.velocity.x -= this.movementForce;  // if a key is pressed apply defined force in proper direction
        }
...
// create & update are framework specific